    ...
    if mode ~= "DP" then
        ...
        if not found then
            ...
            if not checkIfDirect(outputInventory) == bridgeConnectionType then
                ...
                mode = "NI"
            endalogging = require("src/logging")
            Button = require("src/widgets").Button
            Group = require("src/widgets").Group
            ...
            function prettyJSON (json)
                ...
                for i = 1, #j do
                    if j:sub(i, i) == "{" then
                        ...
                        offset = offset + 1
                        for t = 1, tabs do
                            j = insertAt(j, tab, i + offset)
                            offset = offset + string.len(tab)
                        end
                    elseif j:sub(i, i) == "}" then
                        tabs = tabs - 1
                        j = insertAt(j, "\n", i + offset)
                        offset = offset + 1
                        for t = 1, tabs do
            ...
            function createConfig ()
                ...
                config.lastTab = 0
                config.logging = {}
                config.logging.logFile = "log.log"       -- File to log to
                config.logging.logMode = "overwrite"        -- overwrite or append
                config.logging.logLevel = "INFO"            -- DEBUG, INFO, WARNING, ERROR
                config.logging.logTimeSource = "local"      -- ingame: The in-game time, local: The server time, utc: UTC time
                config.logging.logTimeFormat = true         -- false: 12h, true: 24h
                config.allowedRequests = {}
                config.allowedRequests.enabled = false
                config.allowedRequests.builder = true
                config.wifi = {}
                    ...
                    if allowed then
                        for _, itemRequest in ipairs(request.items) do
                            ...
                            if not skipped then
                                ...
                                local status = "m"
                                if existingItem ~= nil and next(existingItem) ~= nil then
            ...
            function timerEvent()
                ...
                elseif id == timerIntervall then
                    ...
                    logging:ERROR("Unknown timer ID: " .. id .. "timerIntervall: " .. timerIntervall .. "timerUpdate: " .. timerUpdate)
                end
            end
...
function setUpDisplay(mon)
    ...
    -- Requests groups
    widgets.allGroupRequests = Group.new(4, "All", mon)
    logging.log("DEBUG", "Added group: " .. widgets.allGroupRequests.label)
    for i, builder in ipairs(builders) do
        local group = Group.new(4 + i, builder.name .. " (lvl" .. builder.lvl .. ")", mon)
        group.collapsed = true
        widgets[builder.name] = group
    ...
    widgets.saveSettings = Button.new(width - 8, 4, 8, 3, "Save", function () saveConfig(config) end, nil, false, mon)
    widgets.saveSettings.backgroundInactive = colors.blue
    logging.log("DEBUG", "Added button: " .. widgets.saveSettings.label)
    widgets.filterRequests = Button.new(2, 4, 17, 1, "Filter Requests", callbackSettings, nil, true, mon)
    widgets.filterRequests.active = config.allowedRequests.enabled
    logging.log("DEBUG", "Added button: " .. widgets.filterRequests.label)
    widgets.filterBuilders = Button.new(2, 6, 17, 1, "Filter Builders", callbackSettings, nil, true, mon)
    widgets.filterBuilders.active = config.allowedRequests.builder
    logging.log("DEBUG", "Added button: " .. widgets.filterBuilders.label)
    widgets.wifiSettings = Button.new(2, 8, 6, 1, "WIFI", callbackSettings, nil, true, mon)
    widgets.wifiSettings.active = config.wifi.wifiEnable
    logging.log("DEBUG", "Added button: " .. widgets.wifiSettings.label)
end
                ...
                if bridge.isItemCrafting({fingerprint=item.fingerprint}) then
                    ...
                    if mode == "RS" then
                        ...
                        if status then
                            ...
                            logging.log("DEBUG", "Couldn't get item: " .. item.name .. " (" .. item.fingerprint .. ") | Error: " .. err)
                        end
                    elseif mode == "ME" then
                        if freeCPUs > 0 then
                            local itenName = ""
...
if not startupSuccess then
    ...
    if testPerformance then
        ...
        timeUpdateDisplay = timeUpdateDisplay / i
        i = 0
        logging.log("INFO", "moveItems took: " .. tostring(timeMoveItems) .. "ms avg")
        logging.log("INFO", "moveItems took: " .. tostring(maxTimeMoveItems) .. "ms max")
        logging.log("INFO", "getInputs took: " .. tostring(timeGetInputs) .. "ms avg")
        logging.log("INFO", "getInputs took: " .. tostring(maxTimeGetInputs) .. "ms max")
        logging.log("INFO", "updateDisplay took: " .. tostring(timeUpdateDisplay) .. "ms avg")
        logging.log("INFO", "updateDisplay took: " .. tostring(maxTimeUpdateDisplay) .. "ms max")
    end
end
